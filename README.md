...
Some versions of Tracktion (some versions of Tracktion 7, maybe others) might have an embedded window offset problem where the plugin's gui is misaligned with the Tracktion plugin window and adding -DTRACKTIONWM to the makefiles BUILD_FLAGS,   BUILD_FLAGS_WIN and BUILD_FLAGS_WIN32 lines might help (for Tracktion use only).
