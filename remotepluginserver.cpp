...
RemotePluginServer::RemotePluginServer(std::string fileIdentifiers) :
    ...
    sprintf(tmpFileBase, "/vstrplugin_shy_%s", fileIdentifiers.substr(54, 6).c_str());
    ...
    tryRead(m_controlRequestFd, &m_386run, sizeof(int));
...
RemotePluginServer::dispatchGetSet(int timeout)
{
    ...
    {
    if (fwait(&m_shmControl->runServer386, 50)) {
        if (errno == ETIMEDOUT) {
           return;
        } else {
            if(m_inexcept == 0)
