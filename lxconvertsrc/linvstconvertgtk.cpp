   ...
   while((dentry = readdir(dirlist)) != NULL)
    ...
    if(convertname.find("*.so") != std::string::npos)
    continue;

    if(convertname.find(".so") != std::string::npos)
    {
    convertname.replace(convertname.begin() + convertname.find(".so"), convertname.end(), "*.so");
    }
    else
    continue;
...
int main (int argc, char *argv[])
{
  ...
  spacertext2 = gtk_label_new ("Choose plugin folder");
  spacertext3 = gtk_label_new ("Convert");
  
  folderselect = gtk_file_chooser_button_new ("Choose plugin Folder", GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
