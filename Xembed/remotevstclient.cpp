...
RemoteVSTClient::RemoteVSTClient(audioMasterCallback theMaster) : RemotePluginClient(theMaster)
{
   ...
   char         buffer[256];
    ...
    if (dlltype == 2)
    {
    LinVstName = "/usr/bin/lin-vst-serverxe32.exe";
    ...
    LinVstName = "/usr/bin/lin-vst-serverxe32.exe.so";
    test = std::ifstream(LinVstName.c_str()).good();
    if (!test)
    {
    m_runok = 1;
    ...
    {
    LinVstName = "/usr/bin/lin-vst-serverxe.exe";
    test = std::ifstream(LinVstName.c_str()).good();
    if (!test)
    {
    m_runok = 1;
    ...
    LinVstName = "/usr/bin/lin-vst-serverxe.exe.so";
    test = std::ifstream(LinVstName.c_str()).good();
    if (!test)
    {
    m_runok = 1;
    ...
    close(fd);    
       ...
       if (dlltype == 2)
        ...
        if (dlltype == 2)
        {
            ...
            #ifdef EMBED
#ifdef BITWIG
            if (execlp("/usr/bin/lin-vst-servertrack32-bw.exe", "/usr/bin/lin-vst-servertrack32-bw.exe", argStr, NULL))
#else
        ...
        else
        {
            #ifdef EMBED
#ifdef BITWIG
            if (execlp("/usr/bin/lin-vst-servertrack-bw.exe", "/usr/bin/lin-vst-servertrack-bw.exe", argStr, NULL))
#else
        ...
        if (dlltype == 2)
        {
            ...
            #ifdef EMBED
#ifdef BITWIG
            if (execlp("/usr/bin/lin-vst-server32-bw.exe", "/usr/bin/lin-vst-server32-bw.exe", argStr, NULL))
#else
        ...
        else
        {
            #ifdef EMBED
#ifdef BITWIG
            if (execlp("/usr/bin/lin-vst-server-bw.exe", "/usr/bin/lin-vst-server-bw.exe", argStr, NULL))
#else
