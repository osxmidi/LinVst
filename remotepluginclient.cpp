...
{
    char buf[256];
    RemotePluginOpcode opcode = RemotePluginProcess;
    ...
    write(m_processFd, &opcode, sizeof(RemotePluginOpcode));
    write(m_processFd, &sampleFrames, sizeof(int));
    
    read(m_processResponseFd, buf, sizeof(char));
...
int RemotePluginClient::processVstEvents(VstEvents *evnts)
{
    ...
    int sizeidx = 0;
    char buf[256];
    RemotePluginOpcode opcode = RemotePluginProcessEvents;
