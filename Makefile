...
BUILD_FLAGS  = -fPIC -O2 -DLVRT -DEMBED -DVESTIGE $(CXX_FLAGS)
BUILD_FLAGS_WIN = -O2 -DLVRT -DEMBED -DVESTIGE

LINK_FLAGS   = $(LDFLAGS)

LINK_PLUGIN = -shared -lpthread -ldl -lrt $(LINK_FLAGS)
LINK_WINE   = -no-pie -lpthread -lX11 -ldl -lrt $(LINK_FLAGS)
...
lin-vst-server: lin-vst-server.wine.o remotepluginserver.wine.o paths.wine.o
	$(WINECXX) $^ $(LINK_WINE) -o $@
