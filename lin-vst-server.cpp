...
#endif
{
    ...
    case audioMasterGetTime:
        ...
        {
            remoteVSTServerInstance->writeInt(remoteVSTServerInstance->m_AMResponseFd, opcode);
            remoteVSTServerInstance->writeInt(remoteVSTServerInstance->m_AMResponseFd, value);
            remoteVSTServerInstance->tryRead(remoteVSTServerInstance->m_AMRequestFd, &timeInfo, sizeof(timeInfo));
            // printf("%f\n", timeInfo.sampleRate);
            rv = (long)&timeInfo;
        }
#endif
...
#ifdef AMT
        {
            ...
            if (alive && !exiting && threadrun)
            {
                ...
                *ptr2 = eventnum;
                ...
                remoteVSTServerInstance->writeInt(remoteVSTServerInstance->m_AMResponseFd, opcode);
                remoteVSTServerInstance->writeInt(remoteVSTServerInstance->m_AMResponseFd, value);
                ok = remoteVSTServerInstance->readInt(remoteVSTServerInstance->m_AMRequestFd);
            }
        }
#endif
        break;
    ...
    if (alive && !exiting && threadrun)
    {
        ...
        remoteVSTServerInstance->writeInt(remoteVSTServerInstance->m_AMResponseFd, opcode);
        remoteVSTServerInstance->tryWrite(remoteVSTServerInstance->m_AMResponseFd, &am, sizeof(am));
        int ok2 = remoteVSTServerInstance->readInt(remoteVSTServerInstance->m_AMRequestFd);
/*
