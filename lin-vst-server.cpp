...
#ifdef RINGB
void
RemoteVSTServer::waitForServer3()
{
...
#endif
{
    ...
    remoteVSTServerInstance->writeOpcodering(&remoteVSTServerInstance->m_shmControl3->ringBuffer, (RemotePluginOpcode)opcode);
    remoteVSTServerInstance->writeIntring(&remoteVSTServerInstance->m_shmControl3->ringBuffer, value);
   
    remoteVSTServerInstance->commitWrite(&remoteVSTServerInstance->m_shmControl3->ringBuffer);
    remoteVSTServerInstance->waitForServer3();

    if(remoteVSTServerInstance->timeinfo)
    {
    memcpy(remoteVSTServerInstance->timeinfo, &remoteVSTServerInstance->m_shm3[61440], sizeof(VstTimeInfo));
    ...
    if (alive && !exiting && threadrun)
    {
        ...
        memcpy(&remoteVSTServerInstance->m_shm3[65536], &am, sizeof(am));
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR cmdline, int cmdshow)
{
    ...
    DWORD threadIdg = 0;
    ThreadHandle[2] = CreateThread(0, 0, GetSetThreadMain, 0, 0, &threadIdg);
    if (!ThreadHandle[2])
    {
        cerr << "Failed to create audio thread!" << endl;
        TerminateThread(ThreadHandle[0], 0);
